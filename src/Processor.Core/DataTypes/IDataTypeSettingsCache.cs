...
/// + load state). Deliberately separate from <see cref="IDataTypeSettingsRepository"/> (which only
/// exposes FindBy/FindAll) so caching/refresh never leaks into the message path. Consumed by the
/// refresh background service and the health checks — not by domain code.
/// </summary>
