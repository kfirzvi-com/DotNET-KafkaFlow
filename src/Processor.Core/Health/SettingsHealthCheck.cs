...
/// <summary>
/// Readiness check for the data-type settings snapshot. Unhealthy if it never loaded; Degraded (still
/// ready, but flagged) if the last successful load is older than the staleness threshold — i.e.
/// refreshes have been failing while a stale snapshot is served.
/// </summary>
public class SettingsHealthCheck : IHealthCheck
{
    ...
    public SettingsHealthCheck(IDataTypeSettingsCache cache, IOptions<DataTypeSettingsOptions> options)
    {
        _cache = cache;
        ...
        HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        ...
        var stalenessThreshold = TimeSpan.FromMinutes(Math.Max(1, _options.RefreshMinutes) * 3);
